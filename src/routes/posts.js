...
router.put('/:id', auth, async (req, res) => {
  ...
  if (!post) return res.status(404).json({ message: 'Not found' });
  if (post.author.toString() !== req.user._id) return res.status(403).json({ message: 'Forbidden' });
...
router.post('/:id/like', auth, async (req, res) => {
  ...
  if (hasLiked) {
    ...
    if (post.author.toString() !== req.user._id) {
      const n = await Notification.create({ user: post.author, type: 'like', actor: req.user._id, post: post._id });
      const io = req.app.get('io');
      io.to(`user:${post.author.toString()}`).emit('notification', { _id: n._id, type: 'like', post: post._id });
    }
  }
  await post.save();
  const populated = await Post.findById(post._id).populate('author', 'username avatarUrl');
